...
impl<Args: InteropSend> Delegate<Args> {
  ...
  pub fn get_params(&self) -> Vec<Class> {
    ...
    while let Some(class) = unsafe {
      Class::from_ptr({
        let ptr =
          crate::binds::mono_signature_get_params(sig, &mut iter as *mut usize as *mut *mut c_void);
        if ptr.is_null() {
          null_mut()
        } else {
  ...
  pub fn get_return(&self) -> Class {
    ...
    unsafe {
      Class::from_ptr(crate::binds::mono_class_from_mono_type(ptr)).expect(
        "Got no method return type, but no return type should be signaled by System.Void type!",
      )
    }
  }
  ...
  fn to_mstring(&self) -> Result<Option<MString>, Exception> {
    ...
    let res = unsafe {
      MString::from_ptr(crate::binds::mono_object_to_string(
        self.get_ptr() as *mut _,
        &mut exc as *mut *mut crate::binds::MonoException as *mut *mut crate::binds::MonoObject,
      ))
    };
