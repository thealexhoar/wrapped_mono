use crate::binds::{
  mono_debug_init, mono_jit_cleanup, mono_jit_exec, mono_jit_init, mono_jit_init_version,
  mono_jit_parse_options, MonoDebugFormat_MONO_DEBUG_FORMAT_MONO,
};
...
pub fn parse_options(args: Vec<&str>) -> Result<(), usize> {
  let arg_count = args.len();
  let argc = arg_count as i32;
  let mut cstr_args: Vec<CString> = Vec::new();
  let mut args_vec: Vec<*mut i8> = Vec::with_capacity(arg_count);
  ...
  unsafe {
    for (index, arg) in args.into_iter().enumerate() {
      ...
      args_vec.push(cstr_arg.as_ptr() as *mut i8);
      cstr_args.push(cstr_arg);
    }
  }
  unsafe {
    mono_jit_parse_options(argc, args_vec.as_mut_ptr());
  }
  let _ = &cstr_args;
...
pub fn debug_init() {
  unsafe {
    mono_debug_init(MonoDebugFormat_MONO_DEBUG_FORMAT_MONO);
  }
}
